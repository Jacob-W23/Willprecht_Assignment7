...
app.post('/editStudentByFname', async(req,res) => {
    try{
        let student = await Student.updateOne({queryFname: req.body.fname}
            ,{
              fname: req.body.fname,
              lname: req.body.lname  
            });
...
app.post('/editCourseByCourseName', async(req,res) => {
    try{
        ...
        return res.status(500).json('{Message: Could not edit course by name}');
    }
});
